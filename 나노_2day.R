...
list1[[2]][-9]                                 #두 번째 성분을 9번째 원소 빼고 가져오기(원소자체가 삭제된 것은 아님)
...
temp = read.table('pop_seoul.txt',  
                  ...
                  fileEncoding='UTF-8')
temp
#확장자까지 다 넣어야 함.txt .csv 등
#큰 따옴표, 작은 따옴표 상관없음.

write.csv(pop_seoul, file='aaa1.csv', row.names = F)
...
load("iris.RData")
iris4 <- iris
#여러 데이터를 불러놓았을 때, 다시 일일이 호출하기 귀찮으니까 모든 데이터를 한 덩어리로 묶어서 이미지로 저장함
#image 파일 load는 변수할당 x 
...
# & : 벡터에서의 and 논리 연산자
2==2 & c( 3>4)   #c로 묶어도 안묶어도 괜찮
2==2 & c( 3<4)   #하나라도 false면 false
...
tmp_df$CC <- 1 # tmp_df가 5행이므로 1도 각 행에 한 번씩 모두 들어감.
...
tmp_df[, -1] #첫번째 위치의 컬럼 제외한 결과값 조회
tmp_df <- tmp_df[, -1] #변수에 꼭 할당해주어야 완전히 삭제됨
...
Sample.df[Sample.df$AA == "a","BB"] 
#대괄호 뒤에 데이터 원본(Sample.df)을 다시 불러야 한다

#Type2
# subset(데이터, 조건)
subset(Sample.df, AA == "a")
subset(Sample.df, AA == "a", select=BB)
#AA컬럼의 값중에서 a 와 b의 값만 추출
#괄호 안에서 데이터 원본을 지정하기 때문에 ,뒤에 다시 한 번 호출하지 않아도 된다
?subset
#Type1
Sample.df[Sample.df$AA %in% c("a","b"),]
#%in%은 or이라고 보면 됨. 포함된 모든 걸 가져와라
#%in%은 많이 사용. 중요!
...
a1[a1$Sepal.Length<=5,"Petal.Width"]
sum(a1[a1$Sepal.Length<=5,"Petal.Width"])
sum(subset(a1,Sepal.Length<=5,select="Petal.Width"))
...
## cut( )을 활용한 연령대 변수 추가
## breaks : 구간 경계값
## include.lowest : 첫 경계값 포함 여부. 위의 경우에서는 10에만 적용됨.
...
merge(a3,a2,by='name',all.y=T)

####################################################################################


##다:다 관계??
student<- data.frame(student_code=c(1,1,1,2,3),name=c('철수','철수','철수','영희','미자'),class_code=c('a','b','c','c','c'))
class<- data.frame(class_name=c('국어','영어','수학','수학','수학'),class_code=c('a','b','c','c','c'),student_code=c(1,1,1,2,3))

merge111=merge(student,class,by='student_code')
